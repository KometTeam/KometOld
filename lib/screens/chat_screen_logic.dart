...
extension on _ChatScreenState {
  ...
  String _prepareTextToSend(String original) {
    if (_encryptionConfigForCurrentChat != null &&
      ...
      return ChatEncryptionService.encryptWithPassword(
        ...
        original,
        profileName:
            _encryptionConfigForCurrentChat!.obfuscationProfile,
      );
    }
    return original;
  }
  ...
  Future<void> _loadEncryptionConfig() async {
    try {
      ...
      // Загружаем chat_key в кэш чтобы sync-методы encrypt/decrypt
      // работали без I/O. Если мастер залочен — пропускаем тихо.
      if (config != null && ChatEncryptionService.hasNewKey(config)) {
        try {
          final mgr = MasterKeyManager.instance;
          if (mgr.isUnlocked && await mgr.hasChatKey(widget.chatId)) {
            await mgr.getOrCreateChatKey(widget.chatId);
          }
        } catch (_) {
          // ignore: best-effort
        }
      ...
      if (mounted) {
        _setStateIfMounted(() {
          ...
          _isEncryptionPasswordSetForCurrentChat = config != null &&
              (ChatEncryptionService.hasNewKey(config) ||
                  (config.legacyXorPassword?.isNotEmpty ?? false));
